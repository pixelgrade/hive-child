<?php
/**
 * Hive Child functions and definitions
 *
...
/**
 * Setup Hive Child Theme's textdomain.
 *
 * Declare textdomain for this child theme.
 * Translations can be filed in the /languages/ directory.
 */
function hive_child_theme_setup() {
	load_child_theme_textdomain( 'hive-child-theme', get_stylesheet_directory() . '/languages' );
}
add_action( 'after_setup_theme', 'hive_child_theme_setup' );
...
function hive_child_enqueue_styles() {

	// Here we are adding the child style.css while still retaining 
	// all of the parents assets (style.css, JS files, etc)
	// 
	wp_enqueue_style( 'hive-child-style',
		get_stylesheet_directory_uri() . '/style.css',
		array('hive-style') //make sure the the child's style.css comes after the parents so you can overwrite rules
	);
}

add_action( 'wp_enqueue_scripts', 'hive_child_enqueue_styles' );
...
function hive_child_overwrite_files() {
	...
	// See the parent's function.php -> the hive_scripts_styles() function 
	// for details like resources names
	//
	// Remember that the rest of the static resources will still get loaded like 
	// hive-imagesloaded, hive-hoverintent and hive-velocity

		wp_dequeue_script( 'hive-scripts' );
	...
	// This is not required, but I assume you are not modifying that much :)
		...
		wp_enqueue_script( 'hive-child-scripts',
			get_stylesheet_directory_uri() . '/assets/js/main.js',
			array( 'jquery', 'masonry', 'hive-imagesloaded', 'hive-hoverintent', 'hive-velocity' ),
			'1.0.0', true );
	...
	// Now you can add your own, modified version of the "style.css" file
		...
		wp_enqueue_style( 'hive-child-style',
			get_stylesheet_directory_uri() . '/style.css',
			array('hive-font-awesome-style') //use the same dependencies as the parent because we want to still use FontAwesome icons
		);
}

// Load the files from the function mentioned above:

	add_action( 'wp_enqueue_scripts', 'hive_child_overwrite_files', 11 );
